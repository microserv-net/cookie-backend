...
The contract between `cookie-interface` (the face) and this repository (the
mind). It is mirrored from the frontend's `docs/backend.md`; the two must not
drift, and `tools/conformance.py` is what keeps them honest. It is deliberately small: one POST that streams
