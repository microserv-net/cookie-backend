//! Prompts.
//!
//! In one file because they are the part most likely to be tuned, and
//! scattering them through the orchestrator would make that a
//! search-and-replace exercise.
//!
//! Two rules run through all of them. **Short output**, because every token
//! costs time on this machine and the reply is going to be spoken — a model
//! that writes an essay produces an assistant nobody lets finish a sentence.
//! And **no chain of thought in anything the user sees**: the models may
//! reason, but the activity log shows what is being done, never the thinking.

/// Classification. The capability list is appended at call time.
...
/// Everything spoken aloud goes through something with this system prompt.
...
{\"steps\": [{\"what\": \"<imperative, one line>\", \"done_when\": \"<how to \
tell it worked>\"}], \"say\": \"<one short sentence to speak first>\"}
...
pub const ARCHITECT_REPLAN: &str = "\
...
objective rested on a wrong assumption, say so in `say` and produce a plan \
that checks the assumption first.

Answer with the same JSON object as before.";

/// The worker's contract. The tool list is appended when there are tools.
...
{\"result\": \"<what you did or found, one or two lines>\", \"ok\": true | \
false, \"evidence\": \"<what shows it, or what went wrong>\"}
